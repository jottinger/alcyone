...
public class AlcyoneConnection {
    ...
    public void changeOctave(AlcyoneVector vector) throws IOException {
                 verifyConnection();
            outputStream.write(buildMessage(AlcyoneMessage.MSG_MIDI_OCTAVE_CHANGE, vector));
    }

    public void changeTransposition(AlcyoneVector vector) throws IOException {
        verifyConnection();
        outputStream.write(buildMessage(AlcyoneMessage.MSG_MIDI_TRANSPOSITION_CHANGE, vector));
    }

    public void changeChannel(AlcyoneVector vector) throws IOException {
        verifyConnection();
        outputStream.write(buildMessage(AlcyoneMessage.MSG_MIDI_CHANNEL_CHANGE, vector));
    }
    ...
    private byte[] buildMessage(AlcyoneMessage msg) {
        return new byte[]{(byte) (msg.getMask() & 0xff)};
    }
